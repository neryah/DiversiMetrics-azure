...
class Portfolio:
    def __init__(self, initial_stocks, initial_weights, historical_csv_path="historical_adjusted_prices.csv", risk_free_rate=0.045):
        """
        ...
        self.historical_data = pd.read_csv(historical_csv_path, index_col='Date', parse_dates=True)
        self.stocks = self.historical_data.columns.tolist()
        

        self.sharpe_penalization = 1
        
        with open("stocks_from_portfolio.py", "w") as f:
            f.write(f"tickers = {list(self.stocks)}")
    ...
    def rank_stocks_for_buying(self, buy_amount):
        ...
        for stock in self.stocks:
            if stock in self.portfolio_weights and self.portfolio_weights[stock] > 0.06:
                continue  # Skip if stock is already overweighted, e.g., > 6%
            
            expected_new_std , expected_new_return = self.get_updates_for_buy(stock, buy_amount)
            
            ranking[stock] = (expected_new_return - self.risk_free_rate) / (expected_new_std ** self.sharpe_penalization)
    ...
    def rank_stocks_for_sell(self, sell_amount):
        ...
        for stock in self.portfolio_weights:
            ...
            if sell_amount > stock_weight * self.portfolio_value:
                continue  # Skip if trying to sell more than available
            
            expected_new_std, expected_new_return = self.get_updates_for_sell(stock, sell_amount)
            ranking[stock] = (expected_new_return - self.risk_free_rate) / (expected_new_std ** self.sharpe_penalization) if expected_new_std > 0 else 0
